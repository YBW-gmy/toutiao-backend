...
@router.get("/list")
async def get_news(category_id:int=Query(default=None,alias="categoryId"),
page:int=1,
    page_size:int=Query(default=10,le=100,alias="pageSize"),db: AsyncSession = Depends(get_db)):
...
async def get_news_detail(
    news_id: int = Query(default=None, alias="id"),
    ...
    if user:
        await history.add_browse_history(db, user.id, news_id)
